...
<body>
	...
	<button class="btn btn-info pull-right" id="export">导出投票数据</button>
	  ...
	  <tbody>
	  	<%ArrayList<Teacher> list=(ArrayList<Teacher>)session.getAttribute("FirstResultList");
	  	  for(int i=0;i<list.size();i++){
	  	%>
	    <tr>
	      <td><%=list.get(i).getName() %></td>
	      <td><%=list.get(i).getCollege() %>
	      <td><%=list.get(i).getFirstVote() %></td>
	    </tr>
	    <%} %>
	  </tbody>
	</table>
	...
	<script>
		...
		$(document).ready(function(){
			...
			$("#export").on("click",function(){
				var $str="<iframe src='../ExportFirstVote?college="+$("select").val()+"' style='display:none;'></iframe>";
				$("#frame").html($str);
			});
		});
	</script>
